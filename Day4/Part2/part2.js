...
const answer= Object.entries(guardsRecordsArray).reduce((mostlyAsleepGuard, currentGuard) => {
    ...
    if(mostlyAsleepGuard.id){
        if(Math.max.apply(Math, currentGuard[1].sleepTable) > Math.max.apply(Math, mostlyAsleepGuard.sleepTable)){
            return ({ id: currentGuard[0], totalSleep: currentGuard[1].totalSleep, sleepTable: currentGuard[1].sleepTable, mostlyMinute: returnMostSleptMinute(currentGuard[1].sleepTable) });
        }else{
